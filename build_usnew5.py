...
app  = B.app_source('new5')
data = B.read('data_usnew5.js')
...
import re
head=re.sub(r'<title>[^<]*</title>','<title>Locator X New Corridors</title>',head,1)
for a,b in [
...
open(B.R+'usnew5.html','w').write(B.assemble(head, body, data, app, extra))
open(B.R+'usnew5-standalone.html','w').write(B.standalone(head, body, data, app, extra))
B.report('usnew5.html')
