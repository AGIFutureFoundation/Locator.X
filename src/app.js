...
function lensOf(l){
 ...
 if(mode==='cat' && r){ const c={asset:'--cat1',hack:'--cat2',value:'--cat3',growth:'--cat4',liab:'--cat5',unrated:'--muted'}[r.cat]||'--muted'; return {tier:r.cat==='asset'?0:r.cat==='liab'?2:1, s:r.score, col:css(c)}; }
...
/* THE IMPORT PAGE USED TO SAY "the map ships with 100 public-record properties".

   It was written when one edition held a hundred records and never revisited. The
   Bay Atlas ships 213,381; the largest edition holds 354,260; the smallest, Shelter
   Cove, holds 24. Whatever the number was, it was wrong everywhere by two to four
   orders of magnitude — and unlike the "walkable Bay Area" strings around it, which
   build_state.py rewrites per edition through its regionalisation pairs, this one
   carried no pair and so could not be corrected at build time either.

   A count is exactly the kind of claim this repository refuses to leave written
   down: the header stat, the list count and the buy-box count are all filled from
   allListings() at runtime, and this now is too. */
function shipCount(){
  const el = document.getElementById('dataShipN');
  if(!el) return;
  let n = 0;
  try{ n = allListings().filter(l => l.src !== 'imp').length; }catch(e){ return; }
  el.textContent = n ? fmtN(n) : 'a catalog of';
}

function refresh(){
  shipCount(); renderScopeNote(); linkSync(); dealSync(); lensNote(); /* map layers owned by other modules follow the same filter as the pins */ try{ if(window.LX3D && LX3D.refreshTowers) LX3D.refreshTowers(); }catch(e){} dealsShown = DEALS_PAGE; lensInvalidate(); renderList(); if(mapReady) renderMarkers(); if(state.sel) renderDrawer(); if(window.LXDash && $('#dash').classList.contains('active')) window.LXDash.render(); }
