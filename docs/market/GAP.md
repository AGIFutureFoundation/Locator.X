...
| Capability | Category tools | Locator.X | Status |
|---|---|---|---|
| Deal underwriting (rental, flip, BRRRR, commercial) | DealCheck, BiggerPockets Pro | `src/underwrite.js` (552) buy box, auto pipeline, offer solver; `src/dev.js` (158) conversion math | **ships** |
...
| Strategy switchboard — compare hold / flip / BRRRR / STR on one property | **Partial and scattered.** `src/underwrite.js`, `src/conv.js`, `src/dev.js` and `src/hacks.js` each answer one strategy. They do not answer side by side. This is the single best-supported new feature in the report, and it needs no new data. |
