...
| Source quality shown to the user | *nobody in the register* | `src/evidence.js` (223) grades every county feed; `src/sources.js` (176) catalogues how each source attaches; the **evidence lens** in `src/app.js` (1712) maps the grade across a whole market and names the field that market is missing most | **ships** — this has no equivalent in the category, and it is now spatial rather than per-property |
