...
|---|-------|--------|--------------------|--------------|
| 1 | **Douglas County, NE** groupBy on its use-class field | Douglas County Assessor / DOTComm GIS (find the ArcGIS REST root from the assessor site) | Omaha's rows in [`nebraska.md`](states/coverage/nebraska.md); seeds a new [crosswalk](../crosswalk/usecodes.json) jurisdiction with measured counts; opens the wave-two market | `crosswalk/validate_usecodes.py` gates the new entry; [`class_screen.py`](../scripts/class_screen.py) screens the pull |
...
| 4 | **Lancaster County, NE** groupBy | Lancaster County Assessor / GIS | Lincoln + the UNL campus ring — the `launi` method's second market ([`nebraska.md`](states/coverage/nebraska.md)) | Same crosswalk/screen path as #1 |
