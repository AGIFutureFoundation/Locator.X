...
| **AGI Opportunity Index** | Explainable buy-box and opportunity score | `src/dashboard.js` (392) scoring and ranking, `LXUW.matches` buy box in `src/underwrite.js` (595), `src/standards.js` (205) the Investment Standard written down and testable | **ships** — and it is explainable by construction: `src/evidence.js` grades the inputs and `src/coverage.js` (340) states what the edition cannot answer at all |
