...
| **Underwriting** | Generates initial scenarios, flags assumptions needing human review, produces base / downside / upside cases, compares strategies and capital structures | **Does not make final investment decisions** | `src/underwrite.js` (590) ten financing structures, offer solver and the stress block; `src/switchboard.js` (438) strategy comparison graded by facts; `src/predict.js` (587) forecasts that carry a backtest | **ships** — and the assumption-flagging the brief asks for is already the switchboard's lever list |
