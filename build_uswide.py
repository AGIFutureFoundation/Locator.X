...
app  = B.app_source('wide')
data = B.read('data_uswide.js')
...
import re
head=re.sub(r'<title>[^<]*</title>','<title>Locator X Conversion Stock</title>',head,1)
for a,b in [
...
open(B.R+'uswide.html','w').write(B.assemble(head, body, data, app, extra))
open(B.R+'uswide-standalone.html','w').write(B.standalone(head, body, data, app, extra))
B.report('uswide.html')
