...
- **Nothing securities-adjacent, ever, without counsel.** No return projection stated as
  fact, no solicitation, no fund terms presented as agreed, and never an implication that
  software equity conveys ownership of portfolio property (`docs/company/FUNDING.md`).
...
| Company layer (AGI Corp) | `docs/company/` — positioning, pricing, portfolio strategy, funding boundary, agents; gated by `scripts/validate_company.py` |
