...
pool=json.load(open(R+'uscorridor_pool6.json'))
...
for i,o in enumerate(pool,1):
    ...
    cd=o['src'].split('.')[0] + '.'
    listings.append(dict(id='UN%06d'%i, addr=o['addr'], city=o['city'], zip=o['zip'],
...
region=dict(name='US Corridors — the new five', center=[-95.0, 38.5], zoom=3.9,
...
s=('window.BA=(function(){"use strict";var D='+j(D)+';var R='+j(rows)+';var M='+j(market)
   +';var G='+j(geo)+';var REGION='+j(region)+';var B="2026-09-04-USNEW5";\n'+decoder+'\n})();')
open(R+'raw/data_usnew5.js','w').write(s)
print('raw bytes', len(s))
import sys; sys.path.insert(0,R)
from compress_data import compress
r=compress(R+'raw/data_usnew5.js', R+'data_usnew5.js')
print('data_usnew5.js', round(r['before']/1e6,2),'->',round(r['after']/1e6,2),'MB', r['literals'])
